...
# macOS Build Script for Mozilla/Gecko
...
echo -e "${BLUE}=== macOS Build Script for Mozilla/Gecko ===${NC}"
...
# Set appropriate configuration file
if [[ "$ARCH" == "arm64" ]]; then
    MOZCONFIG="mozconfig-macos-arm64"
    echo -e "${GREEN}Using Apple Silicon (ARM64) configuration${NC}"
elif [[ "$ARCH" == "x86_64" ]]; then
    MOZCONFIG="mozconfig-macos-x86_64"
    echo -e "${GREEN}Using Intel (x86_64) configuration${NC}"
else
